...
int main(void)
{
	...
	while(1)
	{
		...
		if (PWM_CTR<=PWM_VAL) {LEDPORT|=_BV(LEDPIN);} 
		else {LEDPORT&=~_BV(LEDPIN);}
		...
		if (PWM_CTR==0)
		{
			...
			if ((FRAME_CTR & 7)==0) // generate a new random number every 8 cycles. In reality this is most likely bit serial
			{
				RAND=Rand()&0x1f;
				
				if ((RAND&0x0c)!=0) randflag=1; 
				else randflag=0;// only update if valid
			}
