...
unsigned char lcd8544_buff[1000]; // буфер дисплея 65*102/8
...
void lcd8544_putchar(unsigned char px, unsigned char py, unsigned char ch, unsigned char mode) {
	...
	for (x=0; x<6; x++) {
		...
		lcd8544_buff[lcd_YC*102+px]=lcd8544_buff[lcd_YC*102+px] | (temp<<(7-lcd_YP)); 	// печать верхней части символа

	    if (lcd_YP<7) lcd8544_buff[(lcd_YC+1)*102+px]=lcd8544_buff[(lcd_YC+1)*102+px] | (temp>>(lcd_YP+1)); 	// печать нижней части символа
