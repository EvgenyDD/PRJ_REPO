...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
									   //фон    часы   шрифт  фон2   шрифт2  border
...
/* Private macro -------------------------------------------------------------*/
#define triangleDraw(pos,color)	L2F50_Pixel(pos,Yst-2,	color);	\
...
{
#define Yst 55 			//высота вывода времени и даты
#define Xst 15			//ширина вывода часа
#define Xst2 Xst+92		//ширина вывода числа
	...
	for(;;)
	{
		...
		switch (KeyboardRead())
		{
			...
			case UP:
				TD[n]++;
				TimeDate.hour=TD[0]; TimeDate.minute=TD[1]; TimeDate.second=TD[2];
				TimeDate.date=TD[3]; TimeDate.month=TD[4]; TimeDate.year=TD[5];
...
void MainDisplay(void)
{
	...
	L2F50_FloatWDoubleString(1,60, Display.pAct, (Display.pAct/100 < 100)?2:0,
			"P=", "W   ", FONT16x26, colorScheme[scheme][2],colorScheme[scheme][0]);

	L2F50_FloatWDoubleString(1,40, Display.energy, (Display.energy/100 < 100)?2:0,
			"E=", "kWh   ", FONT10x16, colorScheme[scheme][2],colorScheme[scheme][0]);

	L2F50_FloatWDoubleString(1,21, Display.costAmount, (Display.costAmount/100 < 100)?2:0,
			"Cost=", "Rub    ", FONT10x16, colorScheme[scheme][2],colorScheme[scheme][0]);
